...
export default function CoordinatorDashboard() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "settings", "event"), (snap) => {
      ...
      if (prevLeaderboardStatus.current !== null && published !== prevLeaderboardStatus.current) {
        if (published) {
          toast.success("🏆 Leaderboard is now LIVE!", { description: "Rankings are now visible to students." });
        } else {
  ...
  const initials = userProfile?.name?.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) || "CO";
    ...
    <>
      <div className="p-6 flex-1">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div><h2 className="text-sm text-white font-semibold"><GlobalTitle fallback="Ignited Minds" /></h2><p className="text-xs text-gray-400">Coordinator Portal</p></div>
        </div>
      ...
      <div className="p-4">
        <Card className="bg-white/5 border-white/10 p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0">{initials}</div>
            <div className="flex-1 min-w-0">
              <div className="text-sm text-white truncate">{userProfile?.name || "Coordinator"}</div>
        ...
        <div className="bg-black/20 backdrop-blur-md border-b border-white/10 p-4 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs sm:text-sm text-gray-400 hidden sm:block">Manage event attendance and participants</p>
            </div>
            <Badge className="bg-purple-500/20 text-purple-400 border-purple-500/50 shrink-0">Coordinator</Badge>
          </div>
        </div>
